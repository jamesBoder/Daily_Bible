...
export const AccountManagement: React.FC = () => {
  ...
  const handleLinkGoogle = async () => {
    ...
    try {
      ...
      setGoogleError(error.message || "Failed to link Google account");
  ...
  const handleUnlinkGoogle = async () => {
    ...
    try {
      ...
      if (errorMsg.includes("no password set") || errorMsg.includes("password")) {
        setGoogleError("Please set a password before unlinking Google");
      } else {
    ...
    <div className="max-w-4xl mx-auto px-4 py-8 space-y-6">
      ...
      <Card>
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-4 text-center">
          {hasPassword ? "Update your account password" : "Set a password for your account"}
        </p>

        {/* Set Password Form (for OAuth users without password) */}
        {!hasPassword && !showSetPasswordForm && (
          <span className="flex items-center justify-center">
            <Button onClick={() => setShowSetPasswordForm(true)}>Set Password</Button>
          </span>
        )}
      ...
      <Card>
        ...
        <p className="text-gray-600 dark:text-gray-400 mb-4 text-center">
          <strong>⚠️</strong> These actions are permanent and cannot be undone.
        </p>
