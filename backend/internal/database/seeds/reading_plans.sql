...
VALUES
  ...
  ('names-of-god',                'The Names of God',              'Seven days discovering who God is through the names He reveals in Scripture — Creator, the One who sees, Provider, Healer, Almighty, Shepherd, and Banner.', 7,  false, '',          NULL, NULL, false, true, 6,  NOW(), NOW()),
...
-- 2026-07: Names of God is the second free plan (free tier allows 2 active plans).
-- The INSERT above is ON CONFLICT DO NOTHING, so existing databases need this
-- idempotent UPDATE to pick up the change.
UPDATE reading_plans SET requires_premium = false, updated_at = NOW()
WHERE slug = 'names-of-god' AND requires_premium = true;
